...
class TestDataConverters < Minitest::Test
  def setup
    super
    @data   = "Numbers,:integer,1,:float,3.015"
    @parser = CSWat.new(@data)
  ...
  def test_builtin_integer_converter
    ...
    [-5, 1, 10000000000].each do |n|
      assert_equal(n, CSWat::Converters[:integer][n.to_s])
    end

    # does not convert
    (%w{junk 1.0} + [""]).each do |str|
      assert_equal(str, CSWat::Converters[:integer][str])
    end
  end
  ...
  def test_builtin_float_converter
    ...
    [-5.1234, 0, 2.3e-11].each do |n|
      assert_equal(n, CSWat::Converters[:float][n.to_s])
    end

    # does not convert
    (%w{junk 1..0 .015F} + [""]).each do |str|
      assert_equal(str, CSWat::Converters[:float][str])
    end
  end
  ...
  def test_builtin_date_converter
    ...
    assert_instance_of(
      Date,
      CSWat::Converters[:date][@win_safe_time_str.sub(/\d+:\d+:\d+ /, "")]
    )

    # does not convert
    assert_instance_of(String, CSWat::Converters[:date]["junk"])
  end

  def test_builtin_date_time_converter
    # does convert
    assert_instance_of( DateTime,
                        CSWat::Converters[:date_time][@win_safe_time_str] )

    # does not convert
    assert_instance_of(String, CSWat::Converters[:date_time]["junk"])
  end
  ...
  def test_builtin_all_nested_combo_converter
    ...
    @parser =  CSWat.new(@data)                  # reset parser
  ...
  def test_convert_with_custom_code_using_field_info_header
    @parser = CSWat.new(@data, headers: %w{one two three four five})
  ...
  def test_shortcut_interface
    assert_equal( ["Numbers", ":integer", 1, ":float", 3.015],
                  CSWat.parse_line(@data, converters: :numeric) )

    assert_equal( ["Numbers", ":integer", 1, ":float", 3.015],
                  CSWat.parse_line(@data, converters: [:integer, :float]) )

    assert_equal( ["Numbers", :integer, 1, :float, 3.015],
                  CSWat.parse_line(@data, converters: [:numeric, @custom]) )
  end
