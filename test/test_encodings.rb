...
class TestEncodings < Minitest::Test
  ...
  def test_read_with_default_encoding
    ...
    each_encoding do |encoding|
      ...
      begin
        ...
        result = CSWat.read(@temp_csv_path)[0][0]
      ensure
        no_warnings do
  ...
  def test_auto_line_ending_detection
    ...
    encode_for_tests([["a" * 509]], row_sep: "\r\n") do |data|
      assert_equal("\r\n".encode(data.encoding), CSWat.new(data).row_sep)
    end
  end
    ...
    encode_for_tests([%w[abc def]]) do |data|
      %w[col_sep row_sep quote_char].each do |csv_char|
        assert_equal( "|".encode(data.encoding),
                      CSWat.new(data, csv_char.to_sym => "|").send(csv_char) )
      end
    end
  end
  ...
  def test_parser_works_with_encoded_headers
    encode_for_tests([%w[one two three], %w[1 2 3]]) do |data|
      parsed = CSWat.parse(data, headers: true)
  ...
  def test_built_in_converters_transcode_to_utf_8_then_convert
    encode_for_tests([%w[one two three], %w[1 2 3]]) do |data|
      parsed = CSWat.parse(data, converters: :integer)
  ...
  def test_open_allows_you_to_set_encodings
    encode_for_tests([%w[abc def]]) do |data|
      ...
      CSWat.open(@temp_csv_path, "rb:#{data.encoding.name}") do |csv|
        csv.each do |row|
      ...
      CSWat.open(@temp_csv_path, "rb:UTF-32BE:#{data.encoding.name}") do |csv|
        csv.each do |row|
          assert_all?(row, "Wrong data encoding.") {|f| f.encoding == data.encoding}
        end
      end
  ...
  def test_foreach_allows_you_to_set_encodings
    encode_for_tests([%w[abc def]]) do |data|
      ...
      CSWat.foreach(@temp_csv_path, encoding: data.encoding) do |row|
  ...
  def test_read_allows_you_to_set_encodings
    encode_for_tests([%w[abc def]]) do |data|
      # read and write in encoding
      File.open(@temp_csv_path, "wb:#{data.encoding.name}") { |f| f << data }
      rows = CSWat.read(@temp_csv_path, encoding: data.encoding.name)
  ...
  def test_can_write_csv_in_any_encoding
    each_encoding do |encoding|
      ...
      CSWat.open(@temp_csv_path, "wb:#{encoding.name}") do |f|
        data.each { |row| f << row }
      end
      assert_equal(data, CSWat.read(@temp_csv_path, encoding: encoding.name))
    end
  end
  ...
  def test_explicit_encoding
    ...
    s = CSWat.generate(encoding: "Windows-31J") do |csv|
  ...
  def assert_parses(fields, encoding, options = { })
    ...
    parsed   = CSWat.parse(data, options)
    ...
    CSWat.open(@temp_csv_path, "rb:#{encoding}", options) do |csv|
      csv.each_with_index do |row, i|
        assert_equal(fields[i], row)
      end
    end
    begin
      CSWat.open(@temp_csv_path, "rb:#{encoding}:#{__ENCODING__}", options) do |csv|
        csv.each_with_index do |row, i|
    ...
    CSWat.open(@temp_csv_path, options) do |csv|
      csv.each_with_index do |row, i|
        assert_equal(fields[i], row)
      end
    end
