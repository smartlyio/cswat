...
class TestHeaders < Minitest::Test
  ...
  def test_first_row
    [:first_row, true].each do |setting|  # two names for the same setting
      ...
      assert_nothing_raised(Exception) do
        csv = CSWat.parse(@data, headers: setting)
      end

      # first data row - skipping headers
      row = csv[0]
      assert_not_nil(row)
      assert_instance_of(CSWat::Row, row)
  ...
  def test_array_of_headers
    ...
    csv = nil
    assert_nothing_raised(Exception) do
      csv = CSWat.parse(@data, headers: [:my, :new, :headers])
    end
  ...
  def test_csv_header_string_inherits_separators
    ...
    csv = nil
    assert_nothing_raised(Exception) do
      csv = CSWat.parse( @data.tr(",", "|"), col_sep: "|",
  ...
  def test_return_headers
    ...
    csv = nil
    assert_nothing_raised(Exception) do
      csv = CSWat.parse(@data, headers: true, return_headers: true)
    end
  ...
  def test_converters
    ...
    csv = CSWat.parse( data, headers:        true,
                           return_headers: true,
                           converters:     :numeric )
  ...
  def test_builtin_downcase_converter
    csv = CSWat.parse( "One,TWO Three", headers:           true,
                                      return_headers:    true,
  ...
  def test_builtin_converters_with_blank_header
    csv = CSWat.parse( "one,,three", headers:           true,
                                   return_headers:    true,
  ...
  def test_table_support
    ...
    assert_instance_of(CSWat::Table, csv)
  end
  ...
  def test_skip_blanks
    ...
    expected = [%w[1 2 3]]
    CSWat.parse(@data, headers: true, skip_blanks: true) do |row|
  ...
  def test_headers_reader
    # no headers
    assert_nil(CSWat.new(@data).headers)

    # headers
    csv = CSWat.new(@data, headers: true)
  ...
  def test_blank_row_bug_fix
    ...
    CSWat.parse(@data, headers: true) do |row|
      assert_instance_of(CSWat::Row, row)
    end
  end
end
