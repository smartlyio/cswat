...
class TestTable < Minitest::Test
  def setup
    super
    @rows  = [ CSWat::Row.new(%w{A B C}, [1, 2, 3]),
               CSWat::Row.new(%w{A B C}, [4, 5, 6]),
               CSWat::Row.new(%w{A B C}, [7, 8, 9]) ]
    @table = CSWat::Table.new(@rows)

    @header_table = CSWat::Table.new(
      [CSWat::Row.new(%w{A B C}, %w{A B C}, true)] + @rows
    )
  end
  ...
  def test_set_row_or_column
    ...
    @table[3] = CSWat::Row.new(%w[A B C], [13, 14, 15])
  ...
  def test_set_by_col_with_header_row
    r  = [ CSWat::Row.new(%w{X Y Z}, [97, 98, 99], true) ]
    t = CSWat::Table.new(r)
    t.by_col!
    t['A'] = [42]
    assert_equal(['A'], t['A'])
  ...
  def test_each
    ...
    @table.each { |row| assert_instance_of(CSWat::Row, row) }
  ...
  def test_append
    ...
    # Array append
    assert_equal(CSWat::Row.new(%w[A B C], [10, 11, 12]), @table[-1])

    # Row append
    assert_equal(@table, @table << CSWat::Row.new(%w[A B C], [13, 14, 15]))
    assert_equal(CSWat::Row.new(%w[A B C], [13, 14, 15]), @table[-1])
  end
  ...
  def test_delete_with_blank_rows
    data = "col1,col2\nra1,ra2\n\nrb1,rb2"
    table = CSWat.parse(data, :headers => true)
