...
class TestRow < Minitest::Test
  def setup
    super
    @row = CSWat::Row.new(%w{A B C A A}, [1, 2, 3, 4])
  end

  def test_initialize
    # basic
    row = CSWat::Row.new(%w{A B C}, [1, 2, 3])
    assert_not_nil(row)
    assert_instance_of(CSWat::Row, row)
    assert_equal([["A", 1], ["B", 2], ["C", 3]], row.to_a)

    # missing headers
    row = CSWat::Row.new(%w{A}, [1, 2, 3])
    assert_not_nil(row)
    assert_instance_of(CSWat::Row, row)
    assert_equal([["A", 1], [nil, 2], [nil, 3]], row.to_a)

    # missing fields
    row = CSWat::Row.new(%w{A B C}, [1, 2])
    assert_not_nil(row)
    assert_instance_of(CSWat::Row, row)
  ...
  def test_row_type
    # field rows
    row = CSWat::Row.new(%w{A B C}, [1, 2, 3])         # implicit
    assert_not_predicate(row, :header_row?)
    assert_predicate(row, :field_row?)
    row = CSWat::Row.new(%w{A B C}, [1, 2, 3], false)  # explicit
    assert_not_predicate(row, :header_row?)
    assert_predicate(row, :field_row?)

    # header row
    row = CSWat::Row.new(%w{A B C}, [1, 2, 3], true)
  ...
  def test_to_a
    row = CSWat::Row.new(%w{A B C}, [1, 2, 3]).to_a
  ...
  def test_inspect_shows_symbol_headers_as_bare_attributes
    str = CSWat::Row.new(@row.headers.map { |h| h.to_sym }, @row.fields).inspect
    @row.each do |header, field|
  ...
  def test_can_be_compared_with_other_classes
    assert_not_nil(CSWat::Row.new([ ], [ ]), "The row was nil")
  end
