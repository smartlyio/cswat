...
class TestFeatures < Minitest::Test
    ...
    [";", "\t"].each do |sep|
      TEST_CASES.each do |test_case|
        assert_equal( test_case.last.map { |t| t.tr(",", sep) unless t.nil? },
                      CSWat.parse_line( test_case.first.tr(",", sep),
                                      col_sep: sep ) )
      end
    end
    assert_equal([",,,", nil], CSWat.parse_line(",,,;", col_sep: ";"))
  end

  def test_row_sep
    assert_raise(CSWat::MalformedCSVError) do
        CSWat.parse_line("1,2,3\n,4,5\r\n", row_sep: "\r\n")
    end
    assert_equal( ["1", "2", "3\n", "4", "5"],
                  CSWat.parse_line(%Q{1,2,"3\n",4,5\r\n}, row_sep: "\r\n"))
  end

  def test_quote_char
    TEST_CASES.each do |test_case|
      assert_equal( test_case.last.map { |t| t.tr('"', "'") unless t.nil? },
                    CSWat.parse_line( test_case.first.tr('"', "'"),
                                    quote_char: "'" ) )
    end
  end
  ...
  def test_csv_char_readers
    %w[col_sep row_sep quote_char].each do |reader|
      csv = CSWat.new("abc,def", reader.to_sym => "|")
  ...
  def test_row_sep_auto_discovery
    ["\r\n", "\n", "\r"].each do |line_end|
      ...
      discovered = CSWat.new(data).row_sep
      assert_equal(line_end, discovered)
    end

    assert_equal("\n", CSWat.new("\n\r\n\r").row_sep)

    assert_equal($/, CSWat.new("").row_sep)

    assert_equal($/, CSWat.new(STDERR).row_sep)
  end
  ...
  def test_unknown_options
    assert_raise_with_message(ArgumentError, /unknown/) {
      CSWat.new(@sample_data, unknown: :error)
    }
  end
  ...
  def test_skip_blanks
    ...
    @csv  = CSWat.new(@sample_data, skip_blanks: true)
  ...
  def test_csv_behavior_readers
    %w[ unconverted_fields return_headers write_headers
      ...
      assert_not_predicate(CSWat.new("abc,def"), "#{behavior}?", "Behavior defaulted to on.")
      csv = CSWat.new("abc,def", behavior.to_sym => true)
  ...
  def test_converters_reader
    ...
    assert_equal( [:integer],
                  CSWat.new("abc,def", converters: [:integer]).converters )
    ...
    assert_equal( [:integer, :float],
                  CSWat.new("abc,def", converters: :numeric).converters )

    # custom
    csv = CSWat.new("abc,def", converters: [:integer, lambda {  }])
  ...
  def test_header_converters_reader
    ...
    assert_equal([:downcase], CSWat.new("abc,def", hc => [:downcase]).send(hc))

    # just one
    assert_equal([:downcase], CSWat.new("abc,def", hc => :downcase).send(hc))

    # custom
    csv = CSWat.new("abc,def", hc => [:symbol, lambda {  }])
  ...
  def test_failing_to_escape_col_sep_bug_fix
    assert_nothing_raised(Exception) { CSWat.new(String.new, col_sep: "|") }
  end

  # reported by Chris Roos
  def test_failing_to_reset_headers_in_rewind_bug_fix
    csv = CSWat.new("forename,surname", headers: true, return_headers: true)
  ...
  def test_leading_empty_fields_with_multibyte_col_sep_bug_fix
    ...
    1<=>2<=>3
    END_DATA
    parsed = CSWat.parse(data, col_sep: "<=>")
  ...
  def test_gzip_writer_bug_fix
    Tempfile.create(%w"temp .gz") {|tempfile|
      ...
      zipped = nil
      assert_nothing_raised(NoMethodError) do
        zipped = CSWat.new(Zlib::GzipWriter.open(file))
      end
  ...
  def test_inspect_is_smart_about_io_types
    str = CSWat.new("string,data").inspect
    assert_include(str, "io_type:StringIO", "IO type not detected.")

    str = CSWat.new($stderr).inspect
    ...
    Tempfile.create(%w"temp .csv") {|tempfile|
      ...
      str  = CSWat.open(path) { |csv| csv.inspect }
  ...
  def test_version
    assert_not_nil(CSWat::VERSION)
    assert_instance_of(String, CSWat::VERSION)
    assert_predicate(CSWat::VERSION, :frozen?)
    assert_match(/\A\d\.\d\.\d\Z/, CSWat::VERSION)
  end

  def test_accepts_comment_skip_lines_option
    assert_nothing_raised(ArgumentError) do
      CSWat.new(@sample_data, :skip_lines => /\A\s*#/)
    end
  end

  def test_accepts_comment_defaults_to_nil
    c = CSWat.new(@sample_data)
  ...
  def test_requires_skip_lines_to_call_match
    ...
    assert_raise_with_message(ArgumentError, /skip_lines/) do
      CSWat.new(@sample_data, :skip_lines => regex_stub)
    end
  end

  def test_comment_rows_are_ignored
    sample_data = "line,1,a\n#not,a,line\nline,2,b\n   #also,no,line"
    c = CSWat.new sample_data, :skip_lines => /\A\s*#/
  ...
  def test_string_works_like_a_regexp
    sample_data = "line,1,a\n#(not,a,line\nline,2,b\n   also,#no,line"
    c = CSWat.new sample_data, :skip_lines => "#"
    assert_equal [["line", "1", "a"], ["line", "2", "b"]], c.each.to_a
  end
